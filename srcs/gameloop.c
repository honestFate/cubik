...
/*   Updated: 2022/11/01 02:55:13 by gtrinida         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	textured_raycaster(t_mlx *mlx, int x)
{
	...
	step = (double) mlx->textures[mlx->info->tex_n]->height / mlx->info->line_height;
	tex_pos = (mlx->info->draw_start - (double) screenHeight / 2 + (double) mlx->info->line_height / 2) * step; //I guess we
	//need to cast to double here
	while (y < mlx->info->draw_end)
	{
		//так же строка ниже неизвестно верна ли или нет
		mlx->info->tex_y = (int)tex_pos & (mlx->textures[mlx->info->tex_n]->height - 1); //какая-то побитовая херня, что значит не знаю
		tex_pos += step;
		color = mlx->textures[mlx->info->tex_n]->texture[
			mlx->textures[mlx->info->tex_n]->height * mlx->info->tex_y + mlx->info->tex_x];
...
void	ray_calculate(t_pos *info, t_mlx *mlx)
{
	...
	get_tex_n(info);
	info->tex_x = (int)(info->wall_x * (double)(mlx->textures[info->tex_n]->width));
}
